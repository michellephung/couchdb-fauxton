  ...
  var ActiveTasksViewSourceSequence = React.createClass({
    ...
    sequences: function (item) {
      if (_.isNumber(item) || _.isString(item)) {
        return <ComponentsReact.Clipboard textToCopy={item} uniqueKey={item}/>;
      }

      if (_.isArray(item)) {
        return _.map(item, function (seq, i) {
            return <ComponentsReact.Clipboard textToCopy={seq} uniqueKey={i + Math.random(100)} key={i}/>;
          });
      }

      return  <ComponentsReact.Clipboard textToCopy="???" uniqueKey='unknownRevision'/>;
    },
